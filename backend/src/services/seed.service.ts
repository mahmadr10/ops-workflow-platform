...
/**
 * Shared demo-data seeder. Used by:
 *  - `npm run seed` (prisma/seed.ts) for local/Docker setups
 *  - POST /api/system/seed-demo-data for hosts with no shell access (e.g. Render free tier)
 *
 * Safe to call multiple times: every user/workflow/label is an upsert, and the whole seed is
 * skipped if any workflow already exists (see the route in system.routes.ts), so it never
 * duplicates demo items.
 */
export async function seedDemoData(prisma: PrismaClient) {
  ...
  return {
    workflows: ['Recruitment', 'Sales Pipeline', 'Internal Tasks', 'Client Projects', 'Procurement'],
    users: [
      { email: 'admin@digitalsofts.com', role: 'ADMIN' },
      { email: 'manager@digitalsofts.com', role: 'MANAGER' },
      { email: 'employee@digitalsofts.com', role: 'EMPLOYEE' },
      { email: 'viewer@digitalsofts.com', role: 'READONLY' },
    ],
